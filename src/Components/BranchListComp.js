...
import {fetch_branches, delete_branch} from '../Redux/Branch/branchAction'
import { useDispatch } from 'react-redux';

export default function BranchListComp() {
  ...
  const handleDelete = (id) => {
        if (window.confirm(`Are you sure You want to delete branch with id :${id}`)){;
  ...
  useEffect(() => {
    dispatch(fetch_branches(), setLoading(true));
    
  },[]);

  return (
    <Root >
      <Link to="../addbranch" className='btn btn-primary' style={{marginTop:'10px',marginLeft:'10px'}}><AddIcon/>Add</Link>
          ...
          {(rowsPerPage > 0 ? data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage): data).map((val,index) => (
            <tr key={val.id}>
              ...
              <td><button onClick={()=>{handleShow(val)}} className='btn btn-outline-primary btn-sm'><VisibilityIcon/></button>{' '}
                    <Link type='button'  className='btn btn-outline-primary btn-sm' to={`../editbranch/${val.id}`}><EditNoteOutlinedIcon/></Link>{' '}
      ...
      >
        ...
        <Modal.Body>
          ...
          <label>Branch Name : <strong>{show.branchname}</strong></label><br/>
          <label>Contact No: <strong>{show.phone}</strong></label><br/>
          <label>E-mail : <strong>{show.email}</strong></label><br/>
